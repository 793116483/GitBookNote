...
- **2、实现3D 或 2D 绘图的过程**
    ...
    - **(1) 使用 CPU 开始在线程中执行的任务**
        ...
        - 第2步：渲染时，CPU 根据 **绘制图片代码 或 一张图片 进行逻辑数据计算** 并 通过 Quartz 2D(核心绘图) 或 OpenGL 转换 **立体矢量几何图数据(由 圆数据，直线数据 等多边形构成) 与 对应状态数据(颜色描边还是填充，线宽 等)**进行一对一组装成一个**立体几何状态图的数组** 存到 GPU **顶点化缓存区**寄存器中，GPU开始图像处理与渲染工作

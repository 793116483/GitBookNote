...
- **2、实现3D 或 2D 绘图的过程**
    ...
    - **(2) GPU 处理3D和2D图像 与 像素填充过程**
        ...
        - 第3步：GPU 根据几何多边形位置与颜色数据包 进行复杂几何运算后拆成一个个三角形，然后一个接着一个取出多边形进行**顶点化(Vertex Shader)**(就是通过**光照化处理**把立体(或平面)多边形顶点位置 通过模拟物理现像 投影转化成 2D平面三角形的 顶点像素 ，然后使用顶点着色器把平面三角形的三个顶点像素着白色到上下文 ) **构成了3D或2D物体的轮廓**

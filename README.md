        ...
        - **当前线程 执行任务的 方式**
            ...
            - **队列类型**
                ...
                - **串行队列**
                    ...
                    - 让队列中的任务按串行的方式一个一个执行任务，**必须等前面的任务完全执行成功后才取出接下来的任务**。
                    - **注意**：
                        -  就算是**异步的任务**，也是一个一个执行任务，但**会开启一个子线程** ；

                        - **任务1 嵌套 同步任务2，如果两个任务在不同的串行队列是不会阻塞线程，如果两个任务在同一个串行队列是一定会阻塞线程**

                        - **任务1 嵌套 异步任务2 是不会阻塞线程，因为异步任务可以跳过到后面再执行**
                ...
                - **并发队列**
                    ...
                    - **注意**：
                        - **并发队列中的当前同步任务未能执行完的部分放到后面执行，然后可以接着执行接下来的同步任务**

                        - **多个线程并发功能** 只能**在并发队列 且是在 异步(dispatch_async)**函数下**才有效**

                        - **在并发队列中的任务不管是否嵌套任务都不会阻塞线程**
...
- 相关属性
    ...
    - **注意 设置属性时 优化**
        - **图形的生成，避免触发离屏渲染**：**CALayer 的 border、圆角、阴影、遮罩（mask），CAShapeLayer 的矢量图形显示**，通常会**触发离屏渲染**（offscreen rendering），而**离屏渲染通常发生在 GPU 中**。
            - 如：当一个列表视图中出现大量圆角的 CALayer，并且快速滑动时，可以观察到 **GPU 资源已经占满**，而 **CPU 资源消耗很少。这时界面仍然能正常滑动，但平均帧数会降到很低**。

            - 所以当**连续不断绘制不变的内容**为了避免这种情况，可以尝试**开启 CALayer.shouldRasterize = YES 属性**。

                - 1.把**原本离屏渲染的操作转嫁到 CPU 上**去，减轻GPU工作量；
                - 2.在CPU上的离屏渲染内存绘缓存起来，如果**100ms内没用过会自动删除该缓存内容**；
                - 3.这个属性要谨慎用，**用的好性能提升，用的不好则会造成CPU工作量加重**；
                - 4.这个属性相关用法：https://www.jianshu.com/p/aba6b9fa7ef8

        - 对于只需要圆角的某些场合，也可以**用一张圆角图片 代替 属性设置(可以有效防止图像离屏渲染)**覆盖到视图上面。**最彻底的解决办法，就是把需要显示的图形在后台线程绘制为图片，避免使用圆角、阴影、遮罩等属性**。

...
- **2、实现2D绘图的过程**
    - **(1) 使用 CPU 开始在线程中执行的任务**
        ...
        - 第2步：渲染时，CPU 根据 绘制路径 进行控制逻辑处理与数据平面计算 或根据一张图 通过 Quartz 2D(核心绘图) 进行几何位置与颜色数据打包 存到GPU寄存器中

    - **(2) GPU 处理3D和2D图像 与 像素填充过程**

        - 第3步：GPU 根据几何位置与颜色数据包 进行复杂几何运算后拆成一个个三角形，然后一个接着一个取出三角形进行**顶点化**(就是通过**光栅处理**把三角形立体(或平面)位置 通过模拟物理现像转化成 平面三角形的 像素点(且 根据颜色数据确定每个像素点的属性(颜色与深度) ) ，然后使用顶点着色器把平面三角形的三个顶点像素着白色到上下文 ) **构成了3D或2D物体的轮廓**

        - 第4步：**纹理贴图** ；就是把 物体轮廓 上的点按一定规则连接起来，然后在上下文中形成的多边形区域内贴上白色的图片，最终形成黑底白为画的物体

        - 第5步：**像素处理**: 根据每个**像素点**的属性 通过 PixelShader（像素着色器）进行**染色** ；并将染好色的像素添加到 **显存帧缓冲区**
            - 非 Retain 屏 分辨率 [UIScreen mainScreen].scale = 1 ，对应 1个像素点
            - Retain 屏 分辨率 [UIScreen mainScreen].scale = 2 或 3，对应 4 或 9 个像素点

        - 第6步：由ROP（光栅化引擎）最终完成像素的在View上投影输出

        - 注意：
            - 0. GPU 绘图 是基于 OpenGL ES (它是 OpenGL 中绘图的那部分) 绘图

            - 1. GPU的整个图像处理过程 没有占用 CPU 的资源，它们之间是协同工作的；

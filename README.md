...
- **2、实现3D 或 2D 绘图的过程**
    ...
    - **(1) 使用 CPU 开始在线程中执行的任务**
        - 第0步：CPU 调度 主线程 创建一个图开上下文(画板)

        - 第1步：CPU 调度 主线程 执行 绘图代码( UIBezierPath 、Core Graphics 代码)

        - 第2步：渲染时，CPU 根据 **绘制图片代码 或 一张图片 进行逻辑数据计算** 并 通过 Quartz 2D(核心绘图) 转换 **多维矢量几何图数据(如 圆数据，直线数据 等) 与 对应状态数据(颜色描边还是填充，线宽 等)**进行一对一组装成一个**几何状态图数组** 存到 GPU **顶点化缓存区**寄存器中，GPU开始图像处理与渲染工作

    - **(2) GPU 处理3D和2D图像 与 像素点着色 过程**

        ![](./images/GPU工作流程图.png)

        - 第3步：GPU 根据几何状态图数组数据，**高度并发** 取出**数组内的每个立体几何图的顶点位置集合** 进行复杂的**光源化计算** 转成 **2D平面几何图的顶点像素位置集合** 并且 把 该顶点像素位置集合 进行 **顶点着色**，这个过程就是**顶点化(Vertex Shader)**，把数组中所有几何状态图 顶点化后 **就构成了平面物体的轮廓**
        ...
        - 第5步：**像素颜色属性计算**: 根据从CPU传过来的**多边形数据** 通过 PixelShader（像素着色器）获取**RGBA(即 颜色值 与 深度)** ；这个过程是与 前两步(功能是 计算像素的位置) 独立执行互不影响，可以在不同的线程并发执行
        ...
        - 第7步：由显卡的ROP（光栅化引擎，它是**可编程像素着色器(Fragment Shader)** ）完成像素点对应像素位置渲染对应的颜色；然后再进行搞锯齿化 等功能；一系列操作后将 **像素点**添加到**显存像素(帧)缓冲区**

        - 第8步：**由 显卡 从显存像素(帧)缓存区 拿到 像素 并绘制到屏幕上**

        - 注意：
            - 0.GPU 绘图 是基于 OpenGL ES (它是 OpenGL 中绘图的那部分) 绘图
